...
export const RootCauseHeadline: FC<RootCauseHeadlineProps> = ({ headline }) => {
  ...
  const tagsList = isServiceImpacting
    ? [`${StringsUtil.firstToUpperCase(statusInfo)}`, `${headline.defect.serviceCount} Service(s) Degraded`]
    : [`${StringsUtil.firstToUpperCase(statusInfo)}`];
  ...
  return (
    ...
    >
      ...
      <CUIText>{rootCauseSanitizedSentence}</CUIText>
    </div>
  );
};
