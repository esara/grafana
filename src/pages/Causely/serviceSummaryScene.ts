...
export function serviceSummaryScene(templatised = true, seriesToShow = '__server_names') {
  ...
  const serviceSummary = new ServiceSummaryObject({
    entityTypeCounts: [],
  });
  ...
  queryRunner.addActivationHandler(() => {
    const sub = serviceSummary.subscribeToState((newState) => {
      queryRunner.setState({
        queries: [
          {
            ...queryRunner.state.queries[0],
            seriesCount: newState.entityTypeCounts,
          },
        ],
      });
